...
class ProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            ...
            # Пустые анкеты не показываем в каталоге: нужен аватар И хотя бы одна
            # единица контента — опубликованный образ ИЛИ фото в галерее.
            # Прямая ссылка /people/<id> (retrieve) работает.
            from django.db.models import Q, Exists, OuterRef
            from apps.looks.models import Look
            has_look = Look.objects.filter(author_id=OuterRef("user_id"), is_published=True)
            has_photo = ProfilePhoto.objects.filter(profile_id=OuterRef("pk"))
            qs = qs.annotate(_has_look=Exists(has_look), _has_photo=Exists(has_photo)).filter(
                (Q(_has_look=True) | Q(_has_photo=True)) & Q(avatar__isnull=False) & ~Q(avatar="")
            )
