...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # Гайды (тестовые статьи с авторами)
        guides = [
            {"title": "Термоформовка EVA: основы", "cat": "EVA", "author": "evaforge",
             "summary": "Как гнуть EVA феном и строить объёмные детали брони без заломов.",
             "body": (
                 "EVA-фоам — основной материал для косплей-брони. Чтобы деталь держала форму, её нужно нагреть "
                 "и согнуть по болванке.\n\n"
                 "1. Раскрой. Перенесите выкройку на лист 5–10 мм, режьте под углом 45° для аккуратных стыков.\n"
                 "2. Нагрев. Прогрейте феном (или строительным) до 80–100°C — материал станет податливым.\n"
                 "3. Формовка. Прижмите к болванке/колену и держите до остывания — форма зафиксируется.\n"
                 "4. Склейка. Контактный клей на оба края, подождать 1–2 мин, плотно прижать.\n\n"
                 "Частые ошибки: перегрев (пузыри), холодная склейка (отходит), резка тупым ножом (рваный край). "
                 "Меняйте лезвие каждые 10–15 резов.")},
            {"title": "Стайлинг парика под персонажа", "cat": "Парики", "author": "wigmaster",
             "summary": "Подбор волокна, мытьё, укладка и фиксация сложных причёсок.",
             "body": (
                 "Парик — половина образа. Дешёвый блестящий парик сразу выдаёт новичка.\n\n"
                 "Волокно. Берите матовое термостойкое (до 180°C) — его можно укладывать утюжком.\n"
                 "Мытьё. Холодная вода + кондиционер, не тереть, сушить на болванке.\n"
                 "Объём и шипы. Начёс у корней + лак сильной фиксации; для шипов — клей-стик и фен.\n"
                 "Цвет. Тонирование спиртовыми чернилами поверх базового цвета даёт глубину.\n\n"
                 "Совет: фиксируйте готовую укладку лаком слоями, давая каждому высохнуть — держится дольше.")},
            {"title": "Косплей-грим: контуринг и шрамы", "cat": "Грим", "author": "rin_kz",
             "summary": "База, контуринг лица под персонажа и накладные элементы.",
             "body": (
                 "Грим помогает приблизить лицо к персонажу и держится на фото и под софитами.\n\n"
                 "База. Праймер → тон на полтона светлее → пудра для матовости.\n"
                 "Контуринг. Тёмный оттенок в скулы и крылья носа, светлый — на выступающие части.\n"
                 "Глаза. Аниме-взгляд — нижние ресницы карандашом, увеличенный лимбал линзами.\n"
                 "Шрамы/раны. Желатин или нос-вакс, прокрас красным/фиолетовым, закрепить спиртовым фиксатором.\n\n"
                 "Перед сетом сделайте тест-грим за пару дней — проверите стойкость и аллергию.")},
            {"title": "3D-печать реквизита: от STL до покраски", "cat": "3D-печать", "author": "evaforge",
             "summary": "Слайсинг, постобработка швов и покраска напечатанных деталей.",
             "body": (
                 "3D-печать ускоряет изготовление точного реквизита: мечи, маски, значки.\n\n"
                 "Слайсинг. 0.2 мм слой для скорости, 0.12 мм — для лиц и мелочей. Поддержки — только где нужно.\n"
                 "Постобработка. Сошлифуйте слои (220→400→600), зашпатлюйте швы, повторите.\n"
                 "Грунт. Аэрозольный филлер-праймер выявит дефекты — снова шкурим.\n"
                 "Покраска. База из баллона, детали кистью, финиш — матовый/глянцевый лак.\n\n"
                 "Для прочности крупных деталей печатайте с 3–4 периметрами и 15–20% заполнения.")},
        ]
        for g in guides:
            author = User.objects.filter(username=g["author"]).first()
            obj, created = Guide.objects.get_or_create(title=g["title"], defaults={"is_published": True})
            # Заполняем/обновляем как тестовую статью.
            obj.category = g["cat"]; obj.summary = g["summary"]; obj.body = g["body"]
            if author:
                obj.author = author
            obj.save()
            if created:
